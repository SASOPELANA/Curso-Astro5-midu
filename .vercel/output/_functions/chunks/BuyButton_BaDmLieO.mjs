import { c as createComponent, r as renderTemplate, m as maybeRenderHead, a as createAstro, d as addAttribute, b as renderComponent } from './astro/server_B96ay50d.mjs';
import 'kleur/colors';
import 'clsx';
...
}, "/home/sergiodevs/Escritorio/Curso-Astro5-midu/src/components/AmazonLogo.astro", undefined);
...
const $$BuyButton = createComponent(($$result, $$props, $$slots) => {
  ...
  const SPAIN = "ES";
  const contry = Astro2.request.headers.get("x-Vercel-IP-Country") ?? "ES";
  const storeContry = contry === SPAIN ? "spain" : "usa";
  const contryName = contry === SPAIN ? "Espa\xF1ol" : "Estados Unidos";
  const { buy } = Astro2.props;
  const url = buy[storeContry];
  return renderTemplate`${maybeRenderHead()}<a${addAttribute(url, "href")} target="_blank" rel="noopener noreferrer" title="Comprar en Amazon" class="text-balance inline-flex gap-2 items-center bg-yellow-400 hover:bg-yellow-500
...
Comprar en Amazon - ${contryName} ${renderComponent($$result, "AmazonLogo", $$AmazonLogo, {})} </a>`;
}, "/home/sergiodevs/Escritorio/Curso-Astro5-midu/src/components/BuyButton.astro", undefined);

const $$file = "/home/sergiodevs/Escritorio/Curso-Astro5-midu/src/components/BuyButton.astro";
const $$url = undefined;
